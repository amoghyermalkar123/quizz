...
(* State6 [_transition(0)] *)
State6 ==
  raft_test_raft_currentTerm
      = SetAsFun({ <<"n1", 2>>, <<"n2", 1>>, <<"n3", 3>> })
    /\ raft_test_raft_logs
      = SetAsFun({ <<"n1", <<>>>>, <<"n2", <<>>>>, <<"n3", <<>>>> })
    /\ raft_test_raft_role
      = SetAsFun({ <<"n1", Variant("Candidate", [tag |-> "UNIT"])>>,
...
State7 ==
  raft_test_raft_currentTerm
    ...
    /\ raft_test_raft_logs
      = SetAsFun({ <<"n1", <<>>>>, <<"n2", <<>>>>, <<"n3", <<>>>> })
    /\ raft_test_raft_role
      = SetAsFun({ <<"n1", Variant("Leader", [tag |-> "UNIT"])>>,
        <<"n2", Variant("Leader", [tag |-> "UNIT"])>>,
        <<"n3", Variant("Candidate", [tag |-> "UNIT"])>> })
    /\ raft_test_raft_votedFor
      = SetAsFun({ <<"n1", Variant("Some", "n1")>>,
...
================================================================================
(* Created by Apalache on Thu Mar 12 15:07:55 IST 2026 *)
